...
const getSingleAppliedJob = asyncHandler(async (req, res) => {
  ...
  const result = await GetSingleAppliedJobService(
    candidateUserId as string,
    jobId as string
  );
  res.status(200).json({
    success: true,
...
const getApplicationsByJobId = asyncHandler(async (req, res) => {
  ...
  const { jobId } = req.params;
  const validatedQuery = pickValidFields(req.query, APPLICATION_VALID_Fields);
  const result = await GetApplicationsByJobIdService(
    userId as string,
    jobId as string,
    validatedQuery
  );
...
const getSingleApplication = asyncHandler(async (req, res) => {
  ...
  const result = await GetSingleApplicationService(
    userId as string,
...
const updateApplication = asyncHandler(async (req, res) => {
  const { userId, fullName, email } = req.headers;
  const { applicationId } = req.params;
